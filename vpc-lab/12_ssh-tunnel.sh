...
echo ""
echo -e "  ${BOLD}Von deinem Mac (lokal):${NC}"
echo ""
echo -e "  ${CYAN}[1]${NC} SSH auf Public Instanz:"
echo -e "      ${CYAN}ssh -i $PEM_PATH ec2-user@${PUB_IP}${NC}"

if [ -n "$PRIV_IP_TARGET" ]; then
    echo ""
    echo -e "  ${CYAN}[2]${NC} Tunnel starten (HTTP Port 80 + SSH Port 22 gleichzeitig):"
    echo -e "      ${CYAN}ssh -i $PEM_PATH -L 4747:${PRIV_IP_TARGET}:80 -L 4748:${PRIV_IP_TARGET}:22 ec2-user@${PUB_IP} -N &${NC}"
    echo ""
    echo -e "  ${CYAN}[3]${NC} HTTP-Test (erwartet: hello private):"
    echo -e "      ${CYAN}curl http://localhost:4747${NC}"
    echo ""
    echo -e "  ${CYAN}[4]${NC} SSH direkt auf Private Instanz (ueber Tunnel):"
    echo -e "      ${CYAN}ssh -i $PEM_PATH -p 4748 ec2-user@localhost${NC}"
    echo ""
    echo -e "  ────────────────────────────────────────────────"
    echo -e "  ${BOLD}Von der Public Instanz (nach SSH-Login):${NC}"
    echo ""
    echo -e "  ${CYAN}[5]${NC} SSH direkt auf Private:"
    echo -e "      ${CYAN}ssh -i ~/${KEY_NAME}.pem ec2-user@${PRIV_IP_TARGET}${NC}"
    echo ""
    echo -e "  ${CYAN}[6]${NC} HTTP-Test auf Private:"
    echo -e "      ${CYAN}curl http://${PRIV_IP_TARGET}${NC}  ${DIM}→ erwartet: hello private${NC}"
    echo ""
    echo -e "  ${YELLOW}Aktive Tunnel beenden:${NC}"
    echo -e "      ${CYAN}kill \$(lsof -ti:4747) 2>/dev/null; kill \$(lsof -ti:4748) 2>/dev/null${NC}"
fi
echo ""
